...
class TMDBMovieService implements MovieService {
  ...
  Future<Movie> getRecommendedMovie(int rating, int yearsBack, List<Genre> genres, [DateTime? yearsBackFromDate]) async {
    ...
    final rnd = Random();
    final randomMovieEntity = movieEntities[rnd.nextInt(movieEntities.length)];
    final similarMovieEntities = await _movieRepository.getSimilarMovies(randomMovieEntity);

    final similarMovies = similarMovieEntities.map((e) => Movie.fromEntity(e, genres, const [])).toList();
